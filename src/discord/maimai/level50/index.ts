...
import { ApplicationCommandOptionType, AttachmentBuilder, ButtonStyle, ComponentType, type Interaction, MessageFlags } from "discord.js";
import { BaseError, type DataOrError } from "maidraw";
import { Level50Painter } from "maidraw/maimai";
...
export class Level50ChartCommand {
    ...
    static readonly CHAT_COMMAND_HANDLER = Telemetry.discordMiddleware(async (interaction) => {
        ...
        if (interaction.options.getSubcommandGroup() !== "level50") return ResultTypes.IGNORED;

        let result: DataOrError<Buffer>,
            useBrainrot = false;
        const theme = interaction.options.getString("theme", false) || (Util.isAprilFools() ? "salt-2026-portrait" : this.DEFAULT_THEME);
        const level = interaction.options.getNumber("level", true);
        if (level < 1 || level > 15) {
            await interaction.reply({
                content: "Invalid level. Please try again with a level between 1 and 15.",
        ...
        switch (tracker) {
            case "kamai": {
                const { data: score, err: serr } = await kamai.getPlayerLevel50(username, level, page);
                if (serr) {
                    await Util.reportError(interaction, serr);
                    return ResultTypes.ERROR;
                }
                if (score && score.findIndex((v) => v.chart.title === "Baqeela") !== -1) {
                    useBrainrot = true;
                }
                result = await painter.drawWithScoreSource(kamai, { username, level, page }, { theme });
                break;
            }
            case "divingfish": {
                // result = await painter.drawWithScoreSource(divingfish, { username, level, page }, { theme });
                        ...
                        components: [
                            {
                                ...
                                customId: `maimai::level50.page.${tracker},${username},${theme},${level},${page <= 1 ? 1 : page - 1}`,
                                disabled: page <= 1,
                            },
                            {
                                type: ComponentType.Button,
                                label: "Next Page",
                                style: ButtonStyle.Primary,
                                customId: `maimai::level50.page.${tracker},${username},${theme},${level},${page + 1}`,
                            },
                        ],
                    },
                ],
    ...
    static readonly BUTTON_HANDLER = Telemetry.discordMiddleware(async (interaction: Interaction) => {
        if (!interaction.isButton()) return ResultTypes.IGNORED;
        if (interaction.customId.startsWith("maimai::level50.page.")) {
            const [tracker, username, theme, levelRaw, pageRaw] = interaction.customId.replace("maimai::level50.page.", "").split(",");
            const level = parseFloat(levelRaw),
                page = parseInt(pageRaw, 10);
            if (tracker === "kamai" && username && theme && level && page >= 1) {
                if (interaction.message.interactionMetadata?.user.id !== interaction.user.id) {
                    await interaction.reply({
                        content: "You can't change the page of someone else's chart.",
                        flags: [MessageFlags.Ephemeral],
                    });
                    return ResultTypes.IGNORED;
                }
        ...
        return [
            {
                type: 2,
                name: "level50",
                description: "Generate a chart of your best scores in a specific level.",
                        ...
                        options: [
                            {
                                ...
                                description: "The target level for the Level 50 chart.",
                                descriptionLocalizations: {
                                    "zh-CN": "选择生成 L50 图片的目标难度。",
                                    "zh-TW": "選擇生成 Level 50 圖像的目標難度。",
                                },
                                required: true,
                                choices: Level50ChartCommand.levels,
                            },
                            {
                            ...
                            },
                            {
                                ...
                                description: "The page number for the Level 50 chart. (50 scores per page)",
                                descriptionLocalizations: {
                                    "zh-CN": "选择生成 L50 图片的页码。（每页 50 个）",
                                    "zh-TW": "選擇生成 Level 50 圖像的頁碼。（每頁 50 個）",
                                },
                            },
                            {
                                type: ApplicationCommandOptionType.String,
                                ...
                                description: "Choose from a variety of themes for your Level 50 chart.",
                                descriptionLocalizations: {
                                    "zh-CN": "选择 L50 图片的主题。",
                                    "zh-TW": "選擇 Level 50 圖像的主題。",
                                },
                                choices: Level50ChartCommand.themes,
                            },
                            // {
                            //     type: 3,
                            //     name: "version",
