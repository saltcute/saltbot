...
import { Difficulty, LunaticType } from "gcm-database/ongeki";
import type { Chart } from "gcm-database-local/ongeki";
import type { DataOrError } from "maidraw";
import { ChartPainter, type OngekiScoreAdapter } from "maidraw/ongeki";
...
export class ChartQueryCommand {
    ...
    static readonly CHART_PATH = upath.join(this.DATABASE_PATH, "assets", "ongeki", "charts");
    ...
    static readonly CHAT_COMMAND_HANDLER = Telemetry.discordMiddleware(async (interaction) => {
        ...
        const tracker = this.getChoices<"kamai" | "none">(interaction.options.getString("source", false), ["kamai", "none"], "kamai");
        ...
        //     })()
        // );
        let source: OngekiScoreAdapter | null;
        switch (tracker) {
            case "kamai":
    ...
    static readonly AUTOCOMPLETE_HANDLER = async (interaction: Interaction) => {
        ...
        if (interaction.commandName === "geki" && interaction.options.getSubcommand() === "chart") {
            if (!ChartQueryCommand.searchDatabaseLock && focusedValue) {
                const result = ChartQueryCommand.fuse
                    ...
                    .filter((v) => !(v.item.chart.lunatic === LunaticType.REMASTER))
                    .map((v) => {
                        const id = parseInt(v.item.id, 10);
                        return {
                            name: `${v.item.name.length > 50 ? `${v.item.name.slice(0, 50)}...` : v.item.name}${v.item.chart.lunatic === LunaticType.NONE ? "" : " Lunatic"}${v.matches?.[0].value && v.matches[0].key?.toLowerCase().includes("alias") ? `　|「${v.matches[0].value}」` : ""}`,
                            value: id,
                        };
                    })
                    .slice(0, 25);
    ...
    static {
        (async () => {
            ...
            const loadAliases = async () => ChartQueryCommand.OtherSongNameAlias.getAllAlias();
            const otherAlias = await loadAliases();
    ...
    static async getAllSongs(): Promise<Chart[]> {
        const CacheKey = "geki-chart-ongeki-all-songs";
        const cached = await ChartQueryCommand.cache.get(CacheKey);
        ...
        for (const folder of chartFolders) {
            for (const difficulty of Object.values(Difficulty)) {
                if (fs.existsSync(upath.join(ChartQueryCommand.CHART_PATH, folder, `${difficulty}.json`))) {
                    charts.push(require(upath.join(ChartQueryCommand.CHART_PATH, folder, `${difficulty}.json`)));
                    break;
                }
            }
        }
        await ChartQueryCommand.cache.put(CacheKey, charts, 1000 * 60 * 60);
        return charts;
    }
    ...
    static readonly types = [
        {
            ...
            nameLocalizations: { "zh-CN": "B50（b10 + b50 + p50）" },
            value: "refresh",
        },
        {
            name: "Best 55 (New 15 + Best 30 + Recent 10)",
            nameLocalizations: { "zh-CN": "B55（b15 + b30 + r10）" },
            value: "classic",
        },
    ];
            ...
            return [
                {
                    ...
                    options: [
...

                        {
                            ...
                            description: "Choose between generating Best 60 (platinum 50) or Best 55 (recent 10).",
                            descriptionLocalizations: {
                                "zh-CN": "选择生成 b60 (b10 + b50 + p50) 或是 b55 (b15 + b30 + r10)。",
                                "zh-TW": "選擇生成 Best 60 (platinum 50) 或是 Best 55 (recent 10)。",
                            },
...
export namespace ChartQueryCommand {
    export const cache = new Cache("saltbot/ongeki/chart");
    export class OtherSongNameAlias {
        ...
        static async getAllAlias() {
            ...
            for (const locale of ["en", "ko", "ja"]) {
                if (fs.existsSync(upath.join(OtherSongNameAlias.path, locale, "ongeki.tsv"))) {
                    const tsv = fs.readFileSync(upath.join(OtherSongNameAlias.path, locale, "ongeki.tsv"), { encoding: "utf8" });
